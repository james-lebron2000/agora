...
import { loadOrCreateWallet, loadOrCreateMultiChainWallet, refreshBalances, selectOptimalChain, MultiChainWalletManager, CrossChainBridge, EchoSurvivalManager, formatSurvivalReport, shouldAcceptTask } from '@agora/sdk';
/**
...
export class ConsultantAgent {
    ...
    bridge;
    survivalManager;
    lastSurvivalSnapshot = null;
    portfolio = loadAgentPortfolio();
    activeTasks = new Map();
    completedTasks = new Map();
    MARGIN_RATE = 0.20; // 20% consultant margin
    agentId;
    constructor(wallet, multiChainWallet, agentId = 'consultant-agent') {
        this.wallet = wallet;
        ...
        this.survivalManager = new EchoSurvivalManager(agentId, wallet.address, {
    ...
    async receiveTask(taskRequest) {
        ...
        const taskDecision = shouldAcceptTask(this.lastSurvivalSnapshot, taskRequest.budget.toString(), estimatedCost, 0.15 // 15% minimum profit margin
        );
 ...
 */
export async function createConsultantAgent(agentId) {
