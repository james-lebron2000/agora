 ...
 */
export function loadAgentPortfolio(portfolioPath) {
    if (portfolioPath) {
        try {
            const fs = require('fs');
 ...
 */
export function findAgentsByCapability(portfolio, capabilityName) {
    return portfolio.workers.filter(worker => worker.capabilities.some(cap => cap.name.toLowerCase().includes(capabilityName.toLowerCase()) ||
        cap.description.toLowerCase().includes(capabilityName.toLowerCase())));
}
/**
 * Find agents by tags
 */
export function findAgentsByTags(portfolio, tags) {
    return portfolio.workers.filter(worker => tags.some(tag => worker.tags.includes(tag.toLowerCase())));
}
/**
 * Get the best agent for a specific capability (by reliability and price)
 */
export function getBestAgentForCapability(portfolio, capabilityName) {
...
//# sourceMappingURL=agent-portfolio.js.map