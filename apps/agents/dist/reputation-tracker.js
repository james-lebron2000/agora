 ...
 */
import { getOrCreateReputationOracle, getReputationTier, } from '@agora/sdk/reputation';
/**
...
export class ReputationTracker {
    ...
    checkInterval;
    lastAlerts = new Map(); // Prevent alert spam
    constructor(config = {}) {
     ...
     */
    checkThresholds(agentId, reputation) {
     ...
     */
    createAlert(agentId, severity, currentScore, previousScore, delta, recommendations) {
        const alert = {
            id: generateAlertId(),
            agentId,
            severity,
     ...
     */
    generateAlertTitle(severity, delta) {
        switch (severity) {
            case 'critical':
     ...
     */
    generateAlertMessage(agentId, currentScore, previousScore, delta) {
     ...
     */
    generateRecoveryRecommendations(agentId) {
        const reputation = this.oracle.getReputation(agentId);
        if (!reputation)
            return [];
     ...
     * Get alert history
     */
    getAlertHistory(agentId, severity) {
    ...
    generateReport(agentId) {
        ...
        if (trend) {
            ...
            lines.push(`${trendEmoji} 7-Day Trend: ${trend.direction}`, `   Rate: ${trend.ratePerDay > 0 ? '+' : ''}${trend.ratePerDay.toFixed(2)} points/day`, `   Confidence: ${(trend.confidence * 100).toFixed(0)}%`, ``);
        }
        ...
        lines.push(`📈 Tracker Stats:`, `   Monitored Agents: ${stats.monitoredAgents}`, `   Total Alerts: ${stats.totalAlerts}`, `   Generated: ${new Date().toLocaleString()}`);
        return lines.join('\n');
    }
}
 ...
 */
export function getOrCreateReputationTracker(config) {
    if (!globalTracker) {
 ...
 */
export function getReputationTracker() {
    if (!globalTracker) {
 ...
 */
export function resetReputationTracker() {
    if (globalTracker) {
        globalTracker.stop();
    }
    globalTracker = null;
