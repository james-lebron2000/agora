...
function parseParams(params) {
    ...
    const requirements = Array.isArray(params.requirements)
        ? params.requirements.filter((r) => typeof r === 'string')
        : [];
    const text = readText(params.details)
...
- Contingency plans`;
        ...
        if (complexity)
            userPrompt += `Complexity Level: ${complexity}/5\n`;
        if (proof_type)
            userPrompt += `Proof Type: ${proof_type}\n`;
        userPrompt += `\nTask Details:\n${task.input}\n\n`;
...
//# sourceMappingURL=human-task-bridge.js.map