...
import type { EscrowRecord } from '../../../packages/sdk/src/relay.ts';
import {
  encodeRequestId,
  release as releaseOnchain,
  refund as refundOnchain,
  watchEscrowEvents,
} from '../../../packages/sdk/src/escrow.ts';
...
const ESCROW_MODE = (process.env.AGORA_ESCROW_MODE || 'relay').toLowerCase() as 'relay' | 'onchain';
const ESCROW_NETWORK = (process.env.AGORA_ESCROW_NETWORK || 'base-sepolia') as 'base' | 'base-sepolia';
...
const ESCROW_CONTRACT_ADDRESS = process.env.AGORA_ESCROW_CONTRACT_ADDRESS as Address | undefined;
const ESCROW_SELLER_PRIVATE_KEY = process.env.AGORA_ESCROW_SELLER_PRIVATE_KEY as Hex | undefined;
...
export type KimiAgentConfig<TParams> = {
  ...
  buildPrompt: (task: KimiTask<TParams>) => { system: string; user: string };
...
async function releaseEscrow(agent: AgoraAgent, requestId: string, resolution: 'success' | 'refund') {
  if (ESCROW_MODE === 'onchain') {
...
function readMaxCostUsd(payload: Record<string, unknown>): number | undefined {
...
export async function runKimiAgent<TParams>(config: KimiAgentConfig<TParams>) {
  ...
  const pending = new Map<string, KimiTask<TParams>>();
  ...
  void agent.onRequest(async (request) => {
    ...
    if (!intent || !config.intents.includes(intent)) return;
    ...
    await agent.sendOffer(requestId, offerPayload as any, { thread: request.thread?.id });
