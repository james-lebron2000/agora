...
type TranslationRequest = {
  ...
  requestId: string;
  text: string;
  sourceLang?: LanguageSpec;
  targetLang: LanguageSpec;
  charCount: number;
  price: number;
};
...
function normalizeLanguage(input?: string): LanguageSpec | undefined {
...
function readLanguage(params: Record<string, unknown>, keys: string[]): LanguageSpec | undefined {
...
async function main() {
  ...
  void agent.onRequest(async (request) => {
    ...
    if (!intent || !intents.includes(intent)) return;
