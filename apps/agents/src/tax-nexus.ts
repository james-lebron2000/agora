...
function parseParams(params: Record<string, unknown>): TaxParams | null {
  ...
  const text =
    readText(params.business_details)
...
- Common pitfalls`;
    ...
    if (country) userPrompt += `Target Country: ${country}\n`;
    if (region) userPrompt += `State/Region: ${region}\n`;
    if (annual_sales) userPrompt += `Annual Sales: ${annual_sales} ${currency}\n`;
    if (transactions) userPrompt += `Transaction Volume: ${transactions}/year\n`;
