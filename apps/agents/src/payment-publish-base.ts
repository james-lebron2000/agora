...
const paymentMode = (process.env.AGORA_PAYMENT_MODE || 'onchain').toLowerCase() as 'onchain' | 'synthetic';
const network = (process.env.AGORA_PAYMENT_NETWORK || 'base-sepolia') as 'base' | 'base-sepolia';
const rpcUrl = process.env.AGORA_PAYMENT_RPC_URL;
const paymentToken = (process.env.AGORA_PAYMENT_TOKEN || 'USDC').toUpperCase() as 'USDC' | 'ETH';
const amount = Number(
  process.env.AGORA_PAYMENT_AMOUNT
...
const buyerPrivateKey = process.env.AGORA_PAYMENT_BUYER_PRIVATE_KEY as `0x${string}` | undefined;
...
function normalizeAddress(value: string | undefined, envName: string): `0x${string}` {
...
function normalizePrivateKey(value: string | undefined): `0x${string}` {
  if (!value) fail('AGORA_PAYMENT_BUYER_PRIVATE_KEY is required');
  const trimmed = value.trim();
...
async function createAgent(name: string, intents: string[], did: string) {
  ...
  if (!reg.ok) fail(`failed to register ${name}: ${reg.error || 'unknown error'}`);
  return agent;
}
  ...
  while (Date.now() < deadline) {
    ...
    if (res.ok) {
      for (const evt of res.events) {
        ...
        const reqId = (evt.payload as any)?.request_id;
        if (reqId !== options.requestId) continue;
        if (options.senderId && evt.sender?.id !== options.senderId) continue;
        return evt;
      }
    }
    await sleep(300);
...
async function main() {
  ...
  const payer = normalizeAddress(
    buyerAddress || (paymentMode === 'synthetic' ? '0x1111111111111111111111111111111111111111' : undefined),
    'AGORA_PAYMENT_BUYER_ADDRESS',
  );
  const payee = normalizeAddress(
    sellerAddress || (paymentMode === 'synthetic' ? '0x2222222222222222222222222222222222222222' : undefined),
    'AGORA_PAYMENT_SELLER_ADDRESS',
  );
