...
export class MixMaster {
   ...
   */
  async separateStems(inputPath: string): Promise<StemSeparationResult> {
    ...
    try {
      ...
      await execAsync(
        `demucs --two-stems=vocals,other "${inputPath}" -o "${outputDir}"`,
   ...
   */
  async autoMix(track1Path: string, track2Path: string): Promise<string> {
   ...
   */
  async smartMaster(inputPath: string): Promise<string> {
   ...
   */
  async getAudioInfo(inputPath: string): Promise<any> {
    try {
