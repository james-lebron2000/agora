...
function parseParams(params: Record<string, unknown>): AuditParams | null {
...
Please provide a thorough security analysis.`;
  ...
  formatOutput: (content, task) => {
    ...
    if (critical > 0) overallSeverity = 'critical';
    else if (high > 0) overallSeverity = 'high';
    else if (medium > 0) overallSeverity = 'medium';
    else if (low > 0) overallSeverity = 'low';
