...
async function example2_getBridgeQuote() {
  ...
  const destinationChain: SupportedChain = 'optimism';
  ...
  const senderAddress = '0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045' as Address; // Example address
...
async function example3_executeBridgeTransfer(bridge: CrossChainBridge) {
  ...
  try {
    ...
    if (result.success) {
      ...
      const history = new BridgeTransactionHistory(
        (await import('viem/accounts')).privateKeyToAccount(PRIVATE_KEY).address
      );
...
async function example5_errorHandling(bridge: CrossChainBridge) {
...
async function example6_multiChainOperations() {
  ...
  for (const operation of ['send', 'swap', 'contract'] as const) {
...
async function example8_multiTokenBridging(bridge: CrossChainBridge) {
  console.log('\n=== Example 8: Multi-Token Bridge Operations ===\n');

  const tokens: SupportedToken[] = ['USDC', 'USDT', 'DAI', 'WETH'];
  const chains: SupportedChain[] = ['base', 'optimism', 'arbitrum'];
  const account = (await import('viem/accounts')).privateKeyToAccount(PRIVATE_KEY);
  ...
  bridge.on('transactionFailed', (data) => {
    const failedData = data as { token: string; error: string };
...
async function example9_tokenSpecificBridging(bridge: CrossChainBridge) {
  ...
  for (const token of ['USDT', 'DAI', 'WETH'] as SupportedToken[]) {
...
export {
  example1_basicSetup,
  example2_getBridgeQuote,
  example3_executeBridgeTransfer,
  example4_transactionHistory,
  example5_errorHandling,
  example6_multiChainOperations,
  example7_completeWorkflow,
  example8_multiTokenBridging,
  example9_tokenSpecificBridging
};