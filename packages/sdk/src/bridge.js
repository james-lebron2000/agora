...
const SUPPORTED_CHAIN_LIST = Object.keys(SUPPORTED_CHAINS);
const ADDRESS_REGEX = /^0x[a-fA-F0-9]{40}$/;
const PRIVATE_KEY_REGEX = /^0x[a-fA-F0-9]{64}$/;
function normalizeAddressInput(address, fieldName) {
...
function normalizeAmountInput(amount, fieldName = 'amount') {
...
function normalizeChainInput(chain, fieldName) {
    if (typeof chain !== 'string' || !SUPPORTED_CHAIN_LIST.includes(chain)) {
        throw new Error(`${fieldName} must be one of: ${SUPPORTED_CHAIN_LIST.join(', ')}`);
    }
    return chain;
}
function normalizeTokenInput(token, fieldName = 'token') {
    if (typeof token !== 'string' || !SUPPORTED_TOKENS.includes(token)) {
...
export class CoinGeckoPriceOracle {
    cache = new Map();
    ...
    async getPrice(token) {
        ...
        try {
            const response = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${coinIds[token]}&vs_currencies=usd`);
...
export class BridgeAnalytics {
     ...
     */
    async calculateUSDValue(token, amount) {
    ...
    async getStatistics() {
        ...
        const completedWithTime = transactions.filter(t => t.status === 'completed' && t.timestamp);
     ...
     */
    async calculateChainStatistics(transactions) {
        const chains = ['ethereum', 'base', 'optimism', 'arbitrum'];
        const stats = {};
        for (const chain of chains) {
            const chainTxs = transactions.filter(t => t.sourceChain === chain || t.destinationChain === chain);
     ...
     */
    async calculateTokenStatistics(transactions) {
        const stats = {};
     ...
     */
    calculateDailyStatistics(transactions) {
     ...
     */
    async trackFee(sourceChain, destinationChain, token, nativeFee, gasPriceGwei) {
        if (!this.config.enableFeeTracking)
            return;
     ...
     */
    getFeeTrends(sourceChain, destinationChain, token, limit = 100) {
     ...
     */
    getAverageFee(sourceChain, destinationChain, token) {
        const trends = this.feeTrends.filter(t => t.sourceChain === sourceChain &&
     ...
     */
    getFeeTrendAnalysis(sourceChain, destinationChain, token) {
        const trends = this.feeTrends.filter(t => t.sourceChain === sourceChain &&
            t.destinationChain === destinationChain &&
            t.token === token).sort((a, b) => a.timestamp - b.timestamp);
        if (trends.length < 2) {
     ...
     */
    getBestTimeToBridge(sourceChain, destinationChain, token) {
        const trends = this.feeTrends.filter(t => t.sourceChain === sourceChain &&
            t.destinationChain === destinationChain &&
            t.token === token);
        if (trends.length === 0) {
    ...
    trackTransaction(data) {
        ...
        if (this.config.enableFeeTracking && data.feeUSD) {
            this.trackFee(data.sourceChain, data.destinationChain, data.token, data.feeUSD, '10' // Default gas price, should be passed from actual transaction
            ).catch(console.error);
        }
    }
    /**
...
export class BridgeTransactionMonitor extends EventEmitter {
    ...
    constructor(sourceChain, logger = defaultLogger, pollingConfig) {
        super();
    ...
    async monitorTransaction(txHash, sourceChain, destinationChain, amount, options) {
        ...
        const timeout = options?.timeout ?? (this.pollingConfig.sourceConfirmationTimeoutMs +
    ...
    async monitorSourceConfirmation(txHash, sourceChain, status, signal) {
        ...
        while (!signal.aborted && Date.now() - startTime < timeout) {
            try {
                ...
                if (consecutiveErrors >= this.pollingConfig.maxRetries) {
                    throw new BridgeError('Failed to confirm source transaction', 'TRANSACTION_TIMEOUT', { chain: sourceChain, txHash, retryable: true });
                }
            }
        ...
        throw new BridgeError('Source confirmation timeout', 'TRANSACTION_TIMEOUT', { chain: sourceChain, txHash, retryable: true });
    }
    /**
    ...
    async monitorMessageDelivery(status, sourceChain, destinationChain, signal) {
        ...
        while (!signal.aborted && Date.now() - startTime < timeout) {
            try {
                ...
                if (consecutiveErrors >= this.pollingConfig.maxRetries) {
                    throw new BridgeError('Failed to verify message delivery', 'MESSAGE_VERIFICATION_FAILED', { chain: destinationChain, txHash: status.txHash, retryable: true });
                }
            }
            try {
                await this.delay(this.pollingConfig.intervalMs, signal);
        ...
        throw new BridgeError('Message delivery timeout', 'TRANSACTION_TIMEOUT', { chain: destinationChain, txHash: status.txHash, retryable: true });
    }
    /**
...
export class BridgeWebSocketManager extends EventEmitter {
    ...
    constructor(url, logger = defaultLogger) {
        super();
        this.url = url;
        this.logger = logger;
    }
    ...
    handleReconnect() {
        ...
        const delay = Math.min(this.reconnectDelay * Math.pow(2, this.reconnectAttempts - 1), this.MAX_RECONNECT_DELAY);
 ...
 */
export function createBridgeWebSocketClient(url, logger) {
...
export class CrossChainBridge extends EventEmitter {
    ...
    async sendCrossChainMessage(destinationChain, message, options) {
        ...
        try {
            ...
            const quoteResult = await this.quoteCrossChainMessage(validatedDestinationChain, message, options);
