...
let lastXHRInstance: MockXHRInstance | null = null;
...
describe('ProfileCache', () => {
  ...
  beforeEach(() => {
    ...
    (ProfileCache as unknown as { instance: ProfileCache | null }).instance = null;
  ...
  describe('set and get', () => {
    it('should store and retrieve data', () => {
      ...
      const retrieved = cache.get<AgentProfile>('profile:123');
...
describe('batchGetProfiles', () => {
  ...
  it('should cache fetched profiles', async () => {
    ...
    expect(cache.get<AgentProfile>('profile:agent-1')).toEqual(profile);
  });
  ...
  it('should throw error when fetch fails', async () => {
    ...
    await expect(
      batchGetProfiles(TEST_API_URL, ['agent-1'], TEST_AUTH_TOKEN)
    ).rejects.toThrow('Batch fetch failed: Server Error');
  });
