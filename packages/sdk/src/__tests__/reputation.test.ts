...
// Test constants
const TEST_ADDRESS = '0x742d35Cc6634C0532925a3b8D4e6D3b6e8d3e8D3' as const;
const TEST_ADDRESS_2 = '0x1234567890123456789012345678901234567890' as const;
...
describe('Payment Reliability', () => {
  it('returns neutral baseline for no payments', () => {
    const record: PaymentReliability = {
      totalPayments: 0,
      onTimePayments: 0,
      latePayments: 0,
      failedPayments: 0,
...
describe('Endorsement Value', () => {
  ...
  it('calculates value based on strength', () => {
    ...
    const weak = calculateEndorsementValue({ ...baseEndorsement, strength: 'weak' as const }, DEFAULT_REPUTATION_CONFIG, now);
    const moderate = calculateEndorsementValue({ ...baseEndorsement, strength: 'moderate' as const }, DEFAULT_REPUTATION_CONFIG, now);
    const strong = calculateEndorsementValue({ ...baseEndorsement, strength: 'strong' as const }, DEFAULT_REPUTATION_CONFIG, now);
...
describe('Signal Management', () => {
  let baseReputation: AgentReputation;
...
describe('Reputation Calculation', () => {
  ...
  it('respects time window option', () => {
    const oldSignal: ReputationSignal = {
      type: 'task_completion',
      value: 1,
      weight: 1,
  ...
  it('allows custom weights', () => {
    const customOptions: ReputationCalcOptions = {
      customWeights: {
        task_completion: 0.5,
