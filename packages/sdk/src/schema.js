...
export function validateJsonSchema(schema, value, path = 'root') {
    const errors = [];
    ...
    if (Array.isArray(schemaType)) {
        const anyTypeMatches = schemaType.some((subtype) => validateJsonSchema({ ...schemaObj, type: subtype }, value, path).ok);
        if (!anyTypeMatches)
            errors.push(`${path}: value does not match any allowed type`);
    ...
    if (schemaType === 'array') {
        ...
        if (maxItems != null && value.length > maxItems)
            errors.push(`${path}: expected at most ${maxItems} items`);
        if (schemaObj.items) {
    ...
    if (schemaType === 'object' || schemaObj.properties || schemaObj.required) {
        ...
        for (const key of required) {
            if (!(key in value))
                errors.push(`${path}.${String(key)}: is required`);
        }
