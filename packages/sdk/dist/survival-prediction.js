...
export class SurvivalPredictor {
    ...
    predictTrend(daysAhead = 7) {
        ...
        const regression = this.calculateLinearRegression(this.history.map((h, i) => ({ x: i, y: h.survivalScore })));
        ...
        // Determine trend
        const trend = regression.slope > 0.5 ? 'improving' :
    ...
    predictEconomics(daysAhead = 30) {
        ...
        const balanceRegression = this.calculateLinearRegression(balances.map((b, i) => ({ x: i, y: b })));
        ...
        const runwayRegression = this.calculateLinearRegression(runways.map((r, i) => ({ x: i, y: r })));
        ...
        const burnRateTrend = runwayRegression.slope < -0.1 ? 'increasing' :
    ...
    predictMetric(metric, daysAhead = 7) {
        ...
        switch (metric) {
            ...
            case 'runwayDays':
                return economics.runwayPrediction;
            default:
 ...
 */
export function createPredictorFromSnapshots(snapshots, tasksCompleted = [], tasksFailed = []) {
